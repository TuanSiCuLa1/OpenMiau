...
public class Drag {
    ...
    public void interpolate() {
        if (Math.abs(positionX - targetPositionX) > 0.01F || Math.abs(positionY - targetPositionY) > 0.01F) {
    ...
    public boolean mouseOver(float mouseX, float mouseY) {
        return mouseX >= positionX && mouseX <= positionX + scaleX && mouseY >= positionY && mouseY <= positionY + scaleY;
    }
}
