    ...
    public static String getShaderResource(final String resource) {
        try {
            ...
            if (resourceManager == null) {
                // Return basic built-in shaders for fallback if resource manager isn't ready
                if (resource.equals("vertex.vsh")) {
                    return "#version 120\n" +
                           "void main() {\n" +
            ...
            final InputStream inputStream = resourceManager.getResource(new ResourceLocation("rise/shader/" + resource)).getInputStream();
            ...
            try {
                for (String s; (s = bufferedReader.readLine()) != null; ) {
                    source.append(s).append(System.lineSeparator());
                }
            ...
            System.out.println("An error occurred while getting a shader resource: " + resource);
            e.printStackTrace();
            return null;
        }
    }
