...
public class AntiBot extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        for (EntityPlayer bot : bots) {
            ...
            if (debug.getValue()) {
                ChatUtil.display("%sAntiBot: &fRemoved &r%s &fdue to it being a bot.", bot.getName());
            }
        }
    }
    ...
    private void handleMatrixBot() {
        ...
        if (matrixCollectSample) {
            ...
            for (EntityPlayer bot : bots) {
                ...
                if (debug.getValue()) {
                    ChatUtil.display("%sAntiBot/MatrixBot: &fRemoved &r%s&f. Speed: &b%.2f&f.", bot.getName(), getHorizontalSpeed(bot));
                }
            }
        }
    ...
    private void handleSpawnPlayerPacket(S0CPacketSpawnPlayer packet) {
        ...
        if (killAura != null && killAura.getTarget() != null && !hasRemovedEntities.contains(packet.getEntityID())) {
