...
public class NameTags extends Module {
    ...
    public final BooleanProperty player = new BooleanProperty("player", true, () -> !showTargets.getValue());
    public final BooleanProperty invisibles = new BooleanProperty("invisibles", false, () -> !showTargets.getValue());
    public final BooleanProperty animals = new BooleanProperty("animals", false, () -> !showTargets.getValue());
    public final BooleanProperty mobs = new BooleanProperty("mobs", false, () -> !showTargets.getValue());
    public final BooleanProperty teams = new BooleanProperty("player-teammates", true, () -> !showTargets.getValue());
    ...
    public boolean shouldRenderTags(EntityLivingBase entity) {
        ...
        if (showTargets.getValue()) {
            if (entity instanceof EntityPlayer) {
                return Myau.targetManager != null && Myau.targetManager.isFriend(entity.getName());
            }
            return false;
        }
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        for (Entity entity : mc.theWorld.loadedEntityList) {
            ...
            String nametag = living.getDisplayName().getFormattedText() + " §7[§4❤" + Math.round(living.getHealth()) + "§7]";

            if (showTeam.getValue() && living instanceof EntityPlayer && TeamUtil.isSameTeam((EntityPlayer) living)) {
                nametag = "§a§l" + (shortenedTags.getValue() ? "[TM]" : "[TEAM]") + "§r " + nametag;
            }

            if (showTarget.getValue() && living instanceof EntityPlayer && Myau.targetManager != null && Myau.targetManager.isFriend(living.getName())) {
                nametag = "§4§l" + (shortenedTags.getValue() ? "[T]" : "[TARGET]") + "§r " + nametag;
            }

            if (showFriendTag.getValue() && living instanceof EntityPlayer && Myau.friendManager != null && Myau.friendManager.isFriend(living.getName())) {
            ...
            mc.fontRendererObj.drawString(nametag, centeredPosX + 0.5f, -2.0f, Color.WHITE.getRGB(), true);
