...
public class UserConfigCommand extends Command {
    ...
    public void runCommand(ArrayList<String> args) {
        ...
        if (sub.equals("list") || sub.equals("l")) {
            ...
            if (args.size() < 3) {
                ChatUtil.display(Myau.clientName + "Missing user config id/name&r");
                return;
            }
    ...
    private void listConfigs() {
        try {
            List<OnlineConfigEntry> entries = Collections.unmodifiableList(new ArrayList<>(client.listUserConfigs()));
            runOnClientThread(() -> {
                cache = entries;
                ChatUtil.display(Myau.clientName + (entries.isEmpty() ? "No user configs found&r" : "User configs:&r"));
                for (OnlineConfigEntry entry : entries) {
                    sendEntry(entry);
                }
            });
        } catch (Exception e) {
            runOnClientThread(() -> ChatUtil.sendFormatted(Myau.clientName + "Failed to list user configs: &c" + e.getMessage() + "&r"));
        }
    }
    ...
    private void loadConfig(String input) {
        try {
            ...
            if (entry == null) {
                runOnClientThread(() -> ChatUtil.display(Myau.clientName + "User config not found (&o" + input + "&r)&r"));
                return;
            }
            String json = client.loadUserConfig(entry.getId());
            runOnClientThread(() -> applyConfig(entry, json));
        } catch (Exception e) {
            runOnClientThread(() -> ChatUtil.sendFormatted(Myau.clientName + "Failed to load user config: &c" + e.getMessage() + "&r"));
        }
    }
    ...
    private void showInfo(String input) {
        try {
            ...
            runOnClientThread(() -> {
                if (entry == null) {
                    ChatUtil.display(Myau.clientName + "User config not found (&o" + input + "&r)&r");
                } else {
                    showMetadata(entry);
                }
            });
        } catch (Exception e) {
            runOnClientThread(() -> ChatUtil.sendFormatted(Myau.clientName + "Failed to get user config info: &c" + e.getMessage() + "&r"));
        }
    }
    ...
    private void applyConfig(OnlineConfigEntry entry, String json) {
        try {
            ...
            ChatUtil.display(Myau.clientName + "Failed to load user config: &c" + e.getMessage() + "&r");
        }
    }
    ...
    private void sendEntry(OnlineConfigEntry entry) {
        ...
        String line = String.format("§7» §f%s%s §7[§b%s§7] §7by §a%s", entry.getName(), version, entry.getId(), entry.getAuthor());
    ...
    private void usage() {
        ChatUtil.display(Myau.clientName + "Usage: .userconfig &olist&r | .userconfig &oload&r <&oid/name&r> | .userconfig &oinfo&r <&oid/name&r>");
    }
