    ...
    public void runCommand(ArrayList<String> args) {
        ...
        if (args.size() >= 2) {
            ...
            if (property == null) {
                ...
                ChatUtil.display("%s%s: &o%s&r is set to %s&r (%s)&r", module.getName(), property.getName(), property.formatValue(), property.getValuePrompt()
                );
            } else {
                ...
                try {
                    if (property.parseString(newValue)) {
                        ChatUtil.display(
                                "%s%s: &o%s&r has been set to %s&r", module.getName(), property.getName(), property.formatValue()
                        );
                        return;
                    }
                } catch (Exception e) {
                }
                ChatUtil.display("%sInvalid value for property &o%s&r (%s)&r", property.getName(), property.getValuePrompt()
                );
            }
        } else {
